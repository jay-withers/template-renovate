...
# - see below), defaulting to this template's own single check:
# "pre-commit / Pre-commit".
#
# Check names must match the context a job reports on a PR, not just its job
# id. ci-lint's `pre-commit` job calls a reusable workflow, so the
...
# separated) as you add CI workflows to a repo derived from this template.
# Confirm exact names with: gh pr checks
#
# Env overrides:
